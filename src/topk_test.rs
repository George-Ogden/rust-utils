...
fn test_topk_empty() {
    ...
    assert_eq!(x.into_iter().topk(3).collect_vec(), Vec::new());
}

#[test]
fn test_topk_take_zero() {
    let x: Vec<usize> = vec![3];
    assert_eq!(x.into_iter().topk(0).collect_vec(), Vec::new());
}

#[test]
fn test_topk_take_more() {
    let x: Vec<usize> = vec![0, 1, 2];
    assert_eq!(x.into_iter().topk(5).collect_vec(), vec![0, 1, 2]);
}

#[test]
fn test_topk_take_all_non_unique() {
    let x: Vec<usize> = vec![2, 0, 2, 1, 2, 1];
    assert_eq!(x.into_iter().topk(6).collect_vec(), vec![0, 1, 1, 2, 2, 2]);
}

#[test]
fn test_topk_take_some_non_unique() {
    let x: Vec<usize> = vec![0, 1, 1, 2, 2, 2];
    assert_eq!(x.into_iter().topk(4).collect_vec(), vec![1, 2, 2, 2]);
}
...
fn test_topk_take_some_shuffled(seed: u64) {
    ...
    assert_eq!(x.into_iter().topk(3).collect_vec(), vec![9, 9, 10]);
}
...
fn test_topk_pointers() {
    ...
    assert_eq!(
        Arc::as_ptr(&x.into_iter().topk(1).next().unwrap()),
        Arc::as_ptr(&chosen)
    );
}
...
fn test_topk_by_empty() {
    let x: Vec<usize> = Vec::new();
    assert_eq!(
        x.into_iter().topk_by(3, |x| x + 1).collect_vec(),
        Vec::new()
    );
}
...
fn test_topk_by_take_zero() {
    let x: Vec<usize> = vec![3];
    assert_eq!(
        x.into_iter().topk_by(0, |x| x * 2).collect_vec(),
        Vec::new()
    );
}
...
fn test_topk_by_take_more() {
    let x: Vec<usize> = vec![0, 1, 2];
    assert_eq!(x.into_iter().topk_by(5, |_| 0).collect_vec(), vec![0, 1, 2]);
}
...
fn test_topk_by_reverse_shuffled(seed: u64) {
    ...
    x.shuffle(&mut StdRng::seed_from_u64(seed));
    assert_eq!(
        x.into_iter().topk_by(3, |x| Reverse(*x)).collect_vec(),
        vec![1, 1, 0]
    );
}
...
fn test_topk_by_parity() {
    ...
    assert_eq!(
        x.into_iter().topk_by(4, |x| x % 2).collect_vec(),
        vec![1, 1, 3, 9]
    );
}
...
fn test_topk_by_fn_pointers() {
    ...
    let x = vec![Arc::clone(&chosen), Arc::new(0)];
    assert_eq!(
        Arc::as_ptr(&x.into_iter().topk_by(1, |x| **x).next().unwrap()),
        Arc::as_ptr(&chosen)
    );
}
...
fn test_topk_by_fn_pointers_multiple() {
    ...
    let topk = &mut x.into_iter().topk_by(2, |x| **x);
